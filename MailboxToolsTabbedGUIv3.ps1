...
function ConvertFrom-GridToObjects {
    ...
    #
    # Returns an array of PSCustomObject. Note: NO leading comma on the returns here.
    # ConvertTo-DataTable needs ",$dataTable" to stop PowerShell unrolling the table
    # into DataRows. This function wants the opposite - the caller needs the rows
    # unrolled, and comma-wrapping would hand Export-Csv the List itself (producing a
    # useless Capacity/Count CSV) instead of the rows inside it.
    param(
        [Parameter(Mandatory)]
    ...
    # Resolve column keys once. Access cells by index so this does not depend on
    # DataGridView auto-generating a usable .Name for every column.
    $columnKeys = New-Object System.Collections.Generic.List[string]
    ...
    foreach ($column in $Grid.Columns) {
        ...
        # Guard against duplicate keys, which would throw on the ordered hashtable
        if ($columnKeys.Contains($key)) {
            $key = "$key`_$($column.Index)"
        }
    ...
    foreach ($row in $rows) {
        ...
        for ($i = 0; $i -lt $columnKeys.Count; $i++) {
            $ordered[$columnKeys[$i]] = "$($row.Cells[$i].Value)"
        }
